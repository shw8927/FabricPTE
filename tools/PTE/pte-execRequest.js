            ...
            eh.registerBlockEvent((block) => {
                ...
                if ( inv_m == evtRcvB  ) {
                    if ( IDone == 1 ) {
                        ...
                        logger.error("BLOCK Proposal Response time  number=%d", proposalRespTime.length,proposalRespTime);
                        logger.error("BLOCK Order Response time number =%d",orderRespTime.length,orderRespTime);
    ...
    eventHubs.forEach((eh) => {
        ...
        let txPromise = new Promise((resolve, reject) => {
            ...
            if ( ( IDone == 1 ) && ( inv_m == evtCount )  ) {
                ...
                logger.error("TIMEOUT Proposal Response time  number=%d", proposalRespTime.length,proposalRespTime);
                logger.error("TIMEOUT Order Response time number =%d",orderRespTime.length,orderRespTime);
                ...
                */
                ...
                if (code !== 'VALID') {
                    ...
                    if ( ( IDone == 1 ) && ( inv_m == evtRcv ) ) {
                        ...
                        var  propRespTimeSum=0;
                        var  propRespTimeMax=0;
                        var  propRespTimeMin=999999;
                        ...
                        for(var i=0; i<proposalRespTime.length;i++ ){
                             ...
                             propRespTimeSum +=proposalRespTime[i];

                        }
                        for(var i=0; i<orderRespTime.length;i++ ){
                            if(orderRespTimeMax<orderRespTime[i]) {orderRespTimeMax=orderRespTime[i]};
                            if(orderRespTimeMin>orderRespTime[i]) {orderRespTimeMin=orderRespTime[i]};
